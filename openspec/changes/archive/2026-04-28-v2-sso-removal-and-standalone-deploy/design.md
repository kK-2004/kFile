...
- Safari 浏览器剪贴板复制兼容
- 分享链接使用短码替代 Base64 编码 URL，避免 Cloudflare 520 错误
- CI/CD 自动化（Jenkins + GitHub webhook）
- 凭据安全化，不在代码和 Git 历史中存储密钥
...
**选择**: 新增 `deploy.sh` Jenkins 部署脚本，整合后端 Docker 构建运行、Nginx 重载、旧镜像清理。前端部署独立处理。容器名/镜像名使用 `KFile-v2` / `kfile-v2_i`。

**理由**: 一键部署，减少人工操作遗漏。

### 7. 分享短链接架构

**选择**: 后端存储分享数据到 `share_link` 表（code、data JSON、expireAt），用 UUID→Base62 生成 22 字符短码。分享 URL 格式为 `/share?s={code}`，前端通过 `GET /api/share/{code}` 查询数据。兼容旧的 `?d=` Base64 格式。

**理由**: 原方案将 OSS presigned URL 列表 Base64 编码到查询参数中，URL 过长（数千字符）导致 Cloudflare 520 错误。短码方案将 URL 缩短至约 40 字符。

**备选方案**: 使用服务端 ZIP 打包 + 临时下载链接 → 需要额外存储和清理机制，复杂度高。

### 8. 过期链接清理

**选择**: Spring `@Scheduled` 定时任务，每天 02:00 批量删除 `expire_at < NOW()` 的 share_link 记录。

**理由**: 简单可靠，零外部依赖。过期数据如不清理会无限累积。

**备选方案**: 访问时惰性删除 → 无法清理从未被再次访问的过期链接。

### 9. CI/CD 流程

**选择**: 新增 `Jenkinsfile`，GitHub webhook 触发，仅 `main` 分支执行部署。Jenkins `environment` 块中通过 `credentials()` 注入 OSS_AK、OSS_SK、SPRING_DATASOURCE_USERNAME、SPRING_DATASOURCE_PASSWORD。Deploy stage 直接调用 `deploy.sh`，环境变量自动传递。

**理由**: 变量名与 `deploy.sh` 中的环境变量名一致（`SPRING_DATASOURCE_USERNAME` 等），Jenkins 注入后 `deploy.sh` 直接读取，无需手动 export 映射。

**备选方案**: 在 `sh` 步骤中手动 export 变量映射（如 `DB_USER`→`SPRING_DATASOURCE_USERNAME`）→ 增加维护负担，容易遗漏。

### 10. 凭据安全化

**选择**: 所有密钥（DB 密码、OSS AK/SK）使用 Spring 环境变量占位符（`${SPRING_DATASOURCE_PASSWORD}`、`${OSS_AK}` 等），通过容器 `-e` 参数传入。Git 历史中的泄露密钥用 `git-filter-repo` 替换为占位符。新增 `.dockerignore` 排除非构建文件。

**理由**: 密钥不出现在代码和 Git 历史中，符合安全最佳实践。
...
- **[分享链接存储]** share_link 表数据会增长，但通过每日定时清理过期记录控制
- **[Jenkins 凭据]** 需在 Jenkins 中正确配置 4 个 Secret text credential，变量名大小写敏感（如 `kfile` 不能写成 `KFILE`）
