...
- **WHEN** 执行 `docker build -t kfile:v2 .`
...
### Requirement: 前端 dist 部署到服务器

前端构建产物 SHALL 通过 `rsync` 部署到服务器 `/var/www/k-File/` 目录，由 Nginx 直接服务。

#### Scenario: 前端文件部署

- **WHEN** 执行 `rsync -av --delete frontend/dist/ server:/var/www/k-File/`
- **THEN** 服务器 `/var/www/k-File/` 目录包含最新的前端静态文件
...
`deploy.sh` SHALL 提供一键部署能力：前端 rsync、后端 Docker 构建与运行、Nginx 重载、旧镜像清理。

#### Scenario: 一键部署

- **WHEN** 执行 `bash deploy.sh`
- **THEN** 前端文件同步到 `/var/www/k-File/`，旧容器停止，新镜像构建，新容器在 8081 端口启动，Nginx 配置重载
