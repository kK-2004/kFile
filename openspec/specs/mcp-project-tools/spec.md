# mcp-project-tools Specification

## Purpose

MCP 服务端 SSE 传输与项目/提交相关的 @Tool 工具集。
## Requirements
...
系统 SHALL 通过 MCP 暴露一个未提交者查询工具 `list_missing_submitters`，接收一个项目，返回该项目中尚未提交的允许提交者名单。该工具 SHALL 复用现有未提交者计算逻辑（基于 `allowedSubmitterKeys/List` 与已提交记录）。调用者 SHALL 具有对该项目的管理权限（SUPER 或被分配该项目的 ADMIN），否则 SHALL 返回权限错误。该工具的工具描述 SHALL 明确声明其仅适用于已配置允许提交名单的项目，并 SHALL 引导 agent 在未配置名单或需要查看实际已提交情况时改用 `list_project_submissions`。
...
- **AND** 工具描述 SHALL 引导 agent 改用 `list_project_submissions` 查看实际已提交情况

### Requirement: 项目实际提交名单查询工具

系统 SHALL 通过 MCP 暴露一个实际提交名单查询工具 `list_project_submissions`，按提交者去重（每人仅保留最新一次有效提交，复用与打包/分享相同的去重逻辑），返回每个提交者的 `submitterInfo`、文件名与每个文件的 OSS 实际大小（字节）。该工具 SHALL 适用于所有项目，SHALL NOT 依赖项目是否配置 `allowedSubmitterKeys/List`。调用者 SHALL 具有对该项目的管理权限（SUPER 或被分配该项目的 ADMIN），否则 SHALL 返回权限错误。该工具 SHALL NOT 返回 OSS 预签名下载链接（需打包下载 SHALL 改用 `create_archive_download_link`）。该工具 SHALL 与 `list_missing_submitters` 在工具描述中明确分工：本工具用于查看实际已提交情况，`list_missing_submitters` 仅适用于已配置名单的项目用于查未提交者。

#### Scenario: 有权限用户查询某项目实际提交名单
- **WHEN** SUPER 或被分配某项目的 ADMIN 令牌对该项目调用 `list_project_submissions`
- **THEN** 系统 SHALL 返回按提交者去重后的名单，含每个提交者的 submitterInfo、文件名与文件大小

#### Scenario: 未配置允许提交名单的项目也能返回实际提交者
- **WHEN** 有权限用户对未配置 allowedSubmitterKeys/List 的项目调用 `list_project_submissions`
- **THEN** 系统 SHALL 返回实际已提交的提交者名单（与 `list_missing_submitters` 在此场景返回 enabled=false 的关键区别）

#### Scenario: 无权限用户查询实际提交名单被拒
- **WHEN** 未被分配某项目的 ADMIN 令牌对该项目调用 `list_project_submissions`
- **THEN** 系统 SHALL 返回权限错误，且 SHALL NOT 返回任何名单

#### Scenario: 按提交者字段过滤
- **WHEN** 有权限用户调用 `list_project_submissions` 并提供 fieldKey 与 fieldValue
- **THEN** 系统 SHALL 仅返回其提交者字段以 fieldValue 为前缀的提交者

#### Scenario: 空项目返回空名单
- **WHEN** 有权限用户对无任何提交的项目调用 `list_project_submissions`
- **THEN** 系统 SHALL 返回 totalSubmitters=0 与空 submitters 列表

#### Scenario: 返回数据不含下载链接
- **WHEN** 任意有权限用户调用 `list_project_submissions`
- **THEN** 返回结果 SHALL NOT 包含 OSS 预签名下载链接
- **AND** 需要打包下载时 agent SHALL 改用 `create_archive_download_link`
