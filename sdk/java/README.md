...
- 版本号取自 `sdk/java/pom.xml`，仅允许**正式版本**（无 SNAPSHOT）；**同一版本只发布一次，不可覆盖**（CI 发布前会校验）。升级 SDK = 改 `sdk/java/pom.xml` 版本号 + 发布新版本。
- 本地构建与测试：`mvn -f sdk/java/pom.xml clean verify`。
- 发布由 CI 完成（`.github/workflows/sdk-release.yml`）：在 Actions 页面手动触发并选择 `sdk-java`，或在 `Deploy to Server` 工作流中选择模块 `sdk-java`。发布前自动跑全部测试。
